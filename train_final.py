...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=1280,
        ...
        decoder_embedding = 1280 if decoder_embedding < 0 else decoder_embedding
...
# Convert to PyTorch dataset (entire dataset)
full_dataset = RNASeqDataset(embeddings_np, labels_np)
...
for epoch in tqdm(range(epochs)):
    ...
    for batch in full_dataloader:
        ...
        features = model(data).unsqueeze(1)  # Forward pass
        loss = criterion(features, labels)   # Compute loss
        loss.backward()
        optimizer.step()

        epoch_loss += loss.item()

    print(f'Epoch {epoch + 1}/{epochs}, Loss: {epoch_loss / len(full_dataloader)}')


# Instantiate the classifier
linear_classifier = MLDecoder(num_classes=9, decoder_embedding=1280, initial_num_features=output_dim).to(device)
classifier_optimizer = torch.optim.Adam(linear_classifier.parameters(), lr=1e-4)
...
for epoch in tqdm(range(epochs)):
    ...
    for batch in full_dataloader:
        ...
        with torch.no_grad():
            features = model.encoder(data)  # Extract features using the trained encoder
        
        output = linear_classifier(features.unsqueeze(1))  # Forward pass
        loss = criterion_classifier(output, labels)  # Compute loss
        loss.backward()
